...
# Script to download and load CSV data into PostgreSQL running in Docker


# GitHub raw folder where the CSVs live
GITHUB_RAW_BASE="https://raw.githubusercontent.com/jdbarillas/parchposey/master/data-raw"
...
echo ""
echo "=== Downloading Posey CSV Data ==="
mkdir -p "$DATA_DIR"

for f in "${FILES[@]}"; do
    echo "Downloading $f..."
    curl -fSL -o "$DATA_DIR/$f" "$GITHUB_RAW_BASE/$f"
done

echo ""
echo "✓ All files downloaded into $DATA_DIR"
ls -lh "$DATA_DIR"/*.csv
